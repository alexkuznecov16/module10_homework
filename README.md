...
# JS
Также я в сообщении добавил такие функции как время, тоесть теперь можно посмотреть когда было отправлено сообщение, а также сообщения все сохраняются даже если перезагрузили страницу. + после отправки сообщения скролл будет автоматически делаться вниз.
```
